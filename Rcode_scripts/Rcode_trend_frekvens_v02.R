...
df_obs <- df_obs  %>%
  dplyr::filter(!is.na(Lat_Species)) %>%
  dplyr::filter(!(Lat_Species==0)) %>%
...
# count up the number of attempts to search for the Lat_species, per year, per season
df_srchfor <- df_obs %>%
  group_by(Lat_Species, year, season) %>%
  summarise(n_srchfor=n(), .groups="drop")
# count up the number of actual detections for the Lat_species, per year, per season
...
#
df <- distinct(df_obs, Lat_Species) %>%
...
df_srchfnd <- df_srchfor %>% 
  left_join(df_finds, by=c("Lat_Species", "year", "season")) %>%  
  mutate(n_fnd=ifelse(is.na(n_fnd),0,n_fnd))
#
dff <- df_srchfnd %>%
  group_by(Lat_Species, year,season) %>%
  #summarise(n=n(), f=sum(detected)/n(), .groups="drop") %>%
  mutate(species_season=paste0(Lat_Species,"_",season))
# make a frequency element 'f', that is based on the number of detections
# per Lat_species per year per season, where the 'f' is the number of detections
